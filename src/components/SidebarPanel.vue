<template>
    <aside
        class="flex w-full flex-col gap-4 bg-white/5 p-3 md:rounded-2xl md:border md:border-white/10 md:p-4 md:w-64 lg:w-80"
        ...
        <div class="flex-1 overflow-y-auto thin-scroll space-y-2 px-3">
            <button v-for="note in filteredNotes" :key="note.id"
                ...
                <div class="flex items-center justify-between gap-2">
                    ...
                    <span v-if="note.pinned"
                        class="rounded-full border border-emerald-400/70 px-2 py-px text-[10px] font-semibold text-emerald-200">
                        Pinned
                    </span>
                </div>
